//
//  HeartLungVoiceTableViewController.m
//  HealthStation
//
...
@interface HeartLungVoiceTableViewController ()
...
@implementation HeartLungVoiceTableViewController
...
- (void)createTestModel {
    ...
    for (int i=0; i<20; i++) {
        HeartLungVoiceModel *model = [[HeartLungVoiceModel alloc]init];
        if (i%3 == 0) {
            model.isLungRefer = YES;
            model.voiceRefer = @"肺部杂音：无";
        }else {
            model.isLungRefer = NO;
            model.voiceRefer = @"心脏杂音：无";
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    static NSString *voiceId = @"voiceId";
    HeartLungVoiceTableViewCell *cell;
    // Configure the cell...
    cell = [[HeartLungVoiceTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:voiceId];
    
    HeartLungVoiceModel *model = _modelArr[indexPath.row];
    cell.model = model;
    cell.isLungRefer = model.isLungRefer;
